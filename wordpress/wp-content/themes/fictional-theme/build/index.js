...
class Note {
  ...
  editNote(event) {
    // the edit button
    const editButtonText = event.target.closest("[data-id]").querySelector(`.edit-note i`);
    // shared parent
    const parentContainer = event.target.closest("[data-id]");
    // input fields
    const editable = parentContainer.querySelectorAll(".note-title-field, .note-body-field");
    // button for saving
    const saveButton = parentContainer.querySelector(".update-note");
    // initial state

    if (parentContainer.getAttribute("state") == "readonly") {
      this.makeNoteEditable(editButtonText, parentContainer, editable, saveButton);
    } else {
      this.makeNoteReadonly(editButtonText, parentContainer, editable, saveButton);
      this.getNote(event);
    }
  }
  async saveNote(event) {
    ...
    fetch(`${universityData.root_url}/wp-json/wp/v2/note/${noteID}`, {
      method: "POST",
      // WordPress accepts POST for updates
      headers: {
